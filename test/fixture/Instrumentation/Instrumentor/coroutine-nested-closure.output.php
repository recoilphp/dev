...
function () : Coroutine {
    assert(!\class_exists(\Recoil\Dev\Instrumentation\Trace::class) || ($μ = yield \Recoil\Dev\Instrumentation\Trace::install()) || true); assert(!isset($μ) || $μ->setCoroutine(__FILE__, __LINE__, __CLASS__, __FUNCTION__, '->', \func_get_args()) || true); function () : Coroutine {
    ...
    assert(!isset($μ) || $μ->setLine(__LINE__) || true); yield 3;
};
